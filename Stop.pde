class Stop {
  ...
  void display() {
    ...
    int yOffset = -5;
    if (active || crescActive) yOffset = 5;
    fill(light);
  ...
  void activate(int onoff) {
    if (onoff==1) {
      // if (active==false) {
      sendProgramChange(midiChan, midiNr );  // ON
      active = true;
      //}
    } else { 
      //if (active==true) {
      sendProgramChange(midiChan, midiNr+1 );  //  OFF
      active = false;
      //}
    }
    display();
  ...
  void crescActivate(int onoff) {

    if (active==false) {
      if (onoff==1) {
        if (crescActive==false) {
          sendProgramChange(midiChan, midiNr );  // ON
          crescActive = true;
        }
      } else { 
